...
def car_list_get():
    ...
    response = requests.get('http://127.0.0.1:5000/json/car_list')
    print(response.json())
...
def statistic_history_vehicle_get():
    ...
    response = requests.get('http://127.0.0.1:5000/json/statistic_history_vehicle', params={'startTime':startTime,'endtime':endtime,'type':spanType,'vin':vin})
    print(response.json())
    return jsonify(response.json())